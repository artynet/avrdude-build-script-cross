...
rm -rf avrdude-build/ avrdude-6.2
...
cd avrdude-6.2
for p in ../avrdude-6.2-patches/*.patch; do echo Applying $p; patch -p0 < $p; done
./bootstrap
cd -
...
CC="${CROSS_COMPILE}gcc" CXX="${CROSS_COMPILE}g++" PKG_CONFIG_PATH="$PREFIX/lib/pkgconfig" \
	CFLAGS="-w -O2 $CFLAGS" CXXFLAGS="-w -O2 $CXXFLAGS" LDFLAGS="$LDFLAGS" ../avrdude-6.2/configure $CONFARGS > avrdude.configure.output
