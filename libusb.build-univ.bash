...
then
	...
	then
		wget http://download.sourceforge.net/project/libusb/libusb-1.0/libusb-1.0.21/libusb-1.0.21.tar.bz2
	fi

	tar xfv libusb-1.0.21.tar.bz2
	...
	CC="${CROSS_COMPILE}gcc" CXX="${CROSS_COMPILE}g++" CFLAGS="-w -O2 $CFLAGS" CXXFLAGS="-w -O2 $CXXFLAGS" LDFLAGS="-s $LDFLAGS" ../libusb-1.0.21/configure $CONFARGS
	...
	CONFARGS=" \
		...
		--enable-examples-builds"

	if [[ $CROSS_COMPILE != "" ]] ; then
	  CONFARGS="$CONFARGS --host=$HOST_NATIVE --target=$HOST_NATIVE"
	fi

	CC="${CROSS_COMPILE}gcc" CXX="${CROSS_COMPILE}g++" PKG_CONFIG_PATH="$PREFIX/lib/pkgconfig" CFLAGS="-w -O2 $CFLAGS" \
		CXXFLAGS="-w -O2 $CXXFLAGS" LDFLAGS="-s $LDFLAGS" ../libusb-compat-0.1.5/configure $CONFARGS
...
rm -rf libusb-0.1-build libusb-compat-0.1.5 libusb-1.0.21 libusb-1.0-build
